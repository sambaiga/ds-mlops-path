...
The `grade-predictor` project is now typed and linted. This chapter puts it under version control and connects it to GitHub -- after which [Part 17](05-pytest-testing.qmd) adds a test suite.
...
<code>--global</code> writes to <code>~/.gitconfig</code> and applies to every repository on the machine. Omit it and the setting goes to <code>.git/config</code> inside the current repo only. Use <code>--global</code> for your identity; use local config to override settings for a specific project -- for example, a work email for a client repo.
</div>
...
A 2GB CSV committed by accident lives in history even after <code>git rm</code> -- you have to rewrite history to fully erase it. Secrets committed to a public repo are searchable and must be rotated immediately. Set up <code>.gitignore</code> and run <code>git status</code> before every first commit in a new project. Every time.
</div>
...
`commitizen` enforces this format automatically -- [Part 18](06-pre-commit-automation.qmd) covers setup. For now, write the messages by hand.
...
The `gh` CLI creates the PR without leaving the terminal. CI starts automatically on the push that opens it.
...
**Next:** [Part 17: Testing with pytest](05-pytest-testing.qmd) -- tests for the code you've just versioned.
