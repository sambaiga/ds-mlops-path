...
[![View Source on GitHub](https://img.shields.io/badge/Source-GitHub-181717.svg?logo=github)](https://github.com/sambaiga/ds-mlops-path/blob/main/tutorials/02-dev-tools/04-git-github.qmd)
...
You rename a function and the rest of the pipeline breaks. You want to undo it -- but you saved over the original. Git is your time machine: every commit is a checkpoint you can return to, and every branch is an isolated experiment that only merges when it succeeds. On a team, it is also what keeps two people from overwriting each other's work.

Part 15 (`03-type-annotations.ipynb`) left the `grade-predictor` project typed and linted. This chapter puts it under version control and connects it to GitHub. Part 17 (`05-pytest-testing.qmd`) then adds a test suite to that versioned project.
...
Every data science project eventually breaks in the same three ways: the code changed and nobody knows when, an experiment worked but cannot be reproduced, and two people edited the same file. Git is the tool that prevents all three. This chapter sets up the habits you will use from every chapter onward.
...
![Git three-zone model: Working Tree (files on disk) connects to Staging Area via git add, Staging Area connects to Commit History via git commit. Reverse arrows show git restore for undoing staged or committed changes.](figs/git-areas.svg){fig-alt="Three boxes: Working Tree (purple dashed), Staging Area (blue), Commit History (green). Forward arrows: git add, git commit. Reverse arrows: git restore --staged, git restore."}
...
![Feature branch workflow: a feature branch forks from main, accumulates two commits, then merges back into main. The merge commit is green. The feature branch commits are purple. Main branch commits are blue.](figs/git-branches.svg){fig-alt="Horizontal timeline. Main branch (blue circles): initial setup, merge ridge, update deps. Feature branch (purple circles) forks after initial setup: ridge baseline, parametrize. Merges back at the green merge commit."}
