...
You push a commit. CI fails because ruff found an unused import you missed. You fix it and push again. CI fails again -- the commit message format is wrong. Each round trip costs five minutes and breaks your focus.

`pre-commit` catches all of this the moment you type `git commit`. By the end of this chapter, every quality check from the previous four chapters runs automatically before any commit can land.

Part 19 (`07-pydantic-validation.ipynb`) introduces runtime data validation.
...
`pre-commit` is a framework for managing git hooks. You define your checks in a single YAML file (`.pre-commit-config.yaml`) that's committed with your code. Anyone who clones the repository and runs `pre-commit install` gets the identical set of checks on their machine, each in its own isolated environment with pinned tool versions.

Without pre-commit, quality checks are optional. With pre-commit, they're automatic: the checks run before every commit and block it if they fail. You can't forget to run ruff, and your colleagues can't skip it.
...
Without pre-commit, hooks are raw shell scripts in `.git/hooks/`. They're not version-controlled, so every clone starts with no hooks. Each developer installs them differently, and enforcement is inconsistent.
...
The hooks live in <code>.pre-commit-config.yaml</code> (versioned, shared), not in <code>.git/hooks/</code> (local only). Without running <code>pre-commit install</code>, no hooks execute -- the config file exists but nothing triggers it. Add it to your project README under "Getting started".
</div>
...
- `detect-private-key`: scans for PEM headers and common secret patterns. It's not foolproof, but it catches the common cases.
- `no-commit-to-branch`: prevents direct commits to `main`. All changes go through branches.
- `ruff --fix --exit-non-zero-on-fix`: runs ruff, applies auto-fixes, and exits non-zero if it changed anything. This forces you to stage the fix before re-committing, which keeps the staging area honest.
...
This is the one hook that almost no standard pre-commit tutorial covers, but it's essential for any DS project with Jupyter notebooks.

Jupyter saves cell outputs, plots, and printed values inside the `.ipynb` JSON file. Without stripping them, every re-run of a notebook produces a diff of hundreds of lines even when the code itself hasn't changed. `git diff` becomes unreadable. `git blame` becomes useless. PRs accumulate thousands of lines of changed JSON that nobody reviews.

It's not a nice-to-have. It's the difference between a reviewable diff and a wall of base64.
...
In [Part 16](04-git-github.qmd), you wrote conventional commit messages by hand. Commitizen enforces the format automatically via a `commit-msg` hook, and provides `cz commit` as an interactive alternative when you're not sure what type or scope to use.
...
With this in place, `git commit -m "update stuff"` is blocked. `git commit -m "fix(core): correct weight normalization"` passes.
...
`feat` commits bump the minor version. `fix` commits bump the patch. With `update_changelog_on_bump = true`, the changelog writes itself -- no manual editing.
...
Bumping a version manually means editing <code>pyproject.toml</code>, updating <code>CHANGELOG.md</code>, tagging the commit, and pushing the tag -- four steps, easily forgotten in the wrong order. <code>cz bump</code> does all four in one command, reading the commit history to determine whether this is a major, minor, or patch release.
</div>
...
Hooks can run at different stages, and the choice matters for how much latency you accept:
...
The rule of thumb: if a check takes more than 5 seconds, move it to `pre-push`. Keep `pre-commit` fast so committing often stays frictionless.
...
When a hook fails, git blocks the commit. The hook output explains why -- read the first line: it names the hook, the file, and the rule.
...
`SKIP` is the right tool for work-in-progress commits on a personal branch where you intend to clean up before merging. It bypasses only the named hook -- not all of them.
...
<code>git commit --no-verify</code> bypasses everything at once: commitizen, ruff, detect-private-key, nbstripout, all of it. There's almost no legitimate reason for it on a shared branch. Use <code>SKIP=&lt;hook-id&gt;</code> to bypass only the hook that's blocking you. If you find yourself reaching for <code>--no-verify</code> regularly, the hook is probably too slow or misconfigured -- that's the real problem to fix.
</div>
...
**What's next:** With the full dev tools stack in place, Part 4 covers Classical Machine Learning: building, evaluating, and comparing models on `university_analytics.csv` using the clean, tested, versioned codebase you've built here.
:::
