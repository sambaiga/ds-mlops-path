...
You install `scikit-learn` for one project. Three weeks later a second project breaks because a shared dependency updated. The fix is per-project isolation -- and `uv` delivers it in one tool: project init, dependency tracking, reproducible lockfiles, and script running, all faster than `pip` alone.

Part 12 (`00-vscode-setup.qmd`) set up your editor. The project you build here carries through every remaining Dev Tools chapter -- ruff will lint it in Part 15, git will version it in Part 16, and pytest will test it in Part 17.
...
Ask five Python developers how they manage dependencies and you will get five different answers. Some use pip and venv. Some use Poetry. Some use conda. None of them produce the same environment reliably on a colleague's machine. uv is the answer to that fragmentation: one tool, one configuration file, one command prefix.
